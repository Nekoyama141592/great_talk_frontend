...
import { uidAtom, firstLoadedAtom } from '../../store/atoms'
import { DIKWService, DIKWServiceOptions, UserExperienceResponse } from '../dikw-service'
...
export function useDIKW(options: UseDIKWOptions = {}): DIKWHookReturn {
  ...
  const [lastUserExperience, setLastUserExperience] = useState<UserExperienceResponse | null>(null)
  ...
  const processUserExperience = useCallback(async (
    allPosts: ProcessedPost[],
    userInteractions: ProcessedAIInteraction[],
    followingUsers: ProcessedUser[],
    context: any = {}
  ...
  const getSystemInsights = useCallback(async (
    ...
    try {
      const result = await dikwService.getSystemInsights(users, posts, interactions)
  ...
  }, [options.autoRefresh, options.refreshInterval, refreshData, lastUserExperience])
...
export function useUserDIKW(options: UseDIKWOptions = {}) {
  ...
  const processCurrentUser = useCallback(async (
    allPosts: ProcessedPost[],
    userInteractions: ProcessedAIInteraction[],
    followingUsers: ProcessedUser[],
    context: any = {}
...
export function useBusinessRules(options: UseDIKWOptions = {}) {
  ...
  const checkContentModeration = useCallback((content: ProcessedPost | ProcessedUser) => {
    const result = dikw.dikwService['businessRulesEngine'].executeModerationRules(content)
    setModerationResults(result)
    return result
  }, [dikw.dikwService])

  const executeUserRules = useCallback((user: ProcessedUser, context: any = {}) => {
    return dikw.dikwService['businessRulesEngine'].executeUserRules(user, context)
  }, [dikw.dikwService])

  const executePostRules = useCallback((post: ProcessedPost, context: any = {}) => {
    return dikw.dikwService['businessRulesEngine'].executePostRules(post, context)
  }, [dikw.dikwService])
...
export function useStrategicInsights(options: UseDIKWOptions = {}) {
  ...
  const getStrategicDecisions = useCallback(async (
    users: ProcessedUser[],
    posts: ProcessedPost[],
    interactions: ProcessedAIInteraction[]
  ) => {
    ...
    try {
      const insights = await dikw.getSystemInsights(users, posts, interactions)
  ...
  const getAIAdvice = useCallback(async (domain: string, context: any) => {
