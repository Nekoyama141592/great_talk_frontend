...
import { RawDataEnvelope, RawPagination, RawError, DataSource } from '../models/base'
...
export interface BaseRepository<T, K = string> {
  ...
  findWithPagination(pagination: RawPagination, filters?: FilterOptions<T>): Promise<PaginatedResult<T>>
  count(filters?: FilterOptions<T>): Promise<number>

  // Real-time operations
  subscribe(id: K, callback: (data: RawDataEnvelope<T> | null) => void): () => void
  subscribeToQuery(filter: FilterOptions<T>, callback: (data: RawDataEnvelope<T[]>) => void): () => void
