...
import { BaseRepository, FilterOptions, PaginatedResult, NotFoundError, NetworkError, ValidationError } from '../base'
import { RawUserData, UserDataEnvelope } from '../../models/user'
import { RawDataEnvelope, DataSource, RawPagination } from '../../models/base'

export class FirebaseUserRepository implements BaseRepository<RawUserData, string> {
  ...
  async findMany(filters?: FilterOptions<RawUserData>): Promise<RawDataEnvelope<RawUserData[]>> {
    try {
  ...
  async create(userData: Partial<RawUserData>): Promise<RawDataEnvelope<RawUserData>> {
    try {
      const now = new Date()
  ...
  async update(uid: string, userData: Partial<RawUserData>): Promise<RawDataEnvelope<RawUserData>> {
    try {
      const docRef = doc(db, this.collectionPath, uid)
      const updateData = {
        ...userData,
  ...
  async findByFilter(filter: FilterOptions<RawUserData>): Promise<RawDataEnvelope<RawUserData[]>> {
    ...
    try {
      ...
      return {
        ...
        pagination: {
          ...
          nextCursor: users.length > 0 ? users[users.length - 1].uid : undefined,
        },
      }
    } catch (error) {
      throw this.handleFirestoreError(error)
  ...
  subscribe(uid: string, callback: (data: RawDataEnvelope<RawUserData> | null) => void): () => void {
    const docRef = doc(db, this.collectionPath, uid)
  ...
  private applyFilters(q: any, filters: FilterOptions<RawUserData>): any {
    ...
    if (filters.where) {
      filters.where.forEach(condition => {
        query = where(condition.field as string, condition.operator, condition.value)(query)
      })
    }
