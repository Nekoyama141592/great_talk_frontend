...
import { ProcessedUser, UserNotificationSettings } from '../models/processed-data'
...
export class UserDataProcessor {
  ...
  static processUserData(
    ...
    return {
      ...
      status: {
        ...
        suspensionInfo: rawUser.isSuspended ? this.processSuspensionInfo(rawUser) : undefined,
        verification: {
  ...
  private static processSuspensionInfo(user: RawUserData) {
    ...
    return {
      ...
      until: user.suspendedUntil || new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), // Default 7 days
      appealable: true,
    }
  }
  ...
  private static calculateActivityScore(user: RawUserData): number {
    ...
    return Math.round(factors.posts + factors.followers + factors.following + factors.recency)
  }

  /**
   * Determine user influence level
   */
  private static determineInfluenceLevel(user: RawUserData): 'newcomer' | 'regular' | 'popular' | 'influencer' | 'celebrity' {
  ...
  private static processPrivacySettings(preferences?: RawUserPreferences) {
    ...
    return {
      profileVisibility: preferences.privacy.profileVisibility || defaults.profileVisibility,
      showOnlineStatus: preferences.privacy.showOnlineStatus ?? defaults.showOnlineStatus,
