...
import { Follower, FollowingToken, ApiResult } from '@shared/schema/api-requests'

export class UserRepository {
  /**
   * Follow a user by creating both follower record and following token
   */
  async followUser(currentUserId: string, targetUserId: string): Promise<ApiResult<void>> {
    try {
   ...
   */
  async unfollowUser(currentUserId: string, targetUserId: string): Promise<ApiResult<void>> {
    try {
      const batch = writeBatch(db)
  ...
  async getFollowers(userId: string): Promise<string[]> {
    try {
      const followersQuery = query(collection(db, `public/v1/users/${userId}/followers`))
