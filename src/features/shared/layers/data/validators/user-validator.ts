...
import { ValidationResult } from '@shared/layers/data/models/base'
...
export class UserDataValidator {
  static validateUserData(data: Partial<RawUserData>): ValidationResult {
    ...
    if (
      data.metadata?.email &&
      !this.isValidEmail(data.metadata.email as string)
    ) {
  ...
  static sanitizeUserData(data: Partial<RawUserData>): Partial<RawUserData> {
    ...
    if (data.followerCount !== undefined)
      sanitized.followerCount = Math.max(0, data.followerCount)
    if (data.followingCount !== undefined)
      sanitized.followingCount = Math.max(0, data.followingCount)
    if (data.postCount !== undefined)
      sanitized.postCount = Math.max(0, data.postCount)
    if (data.isOfficial !== undefined)
      sanitized.isOfficial = Boolean(data.isOfficial)
    if (data.isSuspended !== undefined)
      sanitized.isSuspended = Boolean(data.isSuspended)
