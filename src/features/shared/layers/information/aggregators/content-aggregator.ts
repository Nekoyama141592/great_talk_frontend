...
export class ContentAggregator {
  ...
  static aggregateTrendingContent(
    ...
    const recentPosts = posts.filter(
      post => post.metadata.publishedAt >= cutoffDate
    )
  ...
  static aggregateUserAnalytics(
    ...
    const metrics = {
      ...
      aiInteractions: recentPosts.reduce(
        (sum, post) => sum + post.ai.responseCount,
        0
      ),
      engagementReceived: recentPosts.reduce(
        (sum, post) => sum + post.engagement.interactions,
        0
      ),
  ...
  private static isContentFiltered(
    post: ProcessedPost,
    _user: ProcessedUser
  ): boolean {
  ...
  private static calculateTopicGrowth(
    _topic: string,
    _timeframe: string
  ): number {
  ...
  private static calculateFollowerGrowth(
    user: ProcessedUser,
    _timeframe: string
  ): number {
  ...
  private static calculateNewFollowers(
    user: ProcessedUser,
    _timeframe: string
  ): number {
  ...
  private static calculateUserGrowthRate(
    _user: ProcessedUser,
    _timeframe: string
  ): number {
  ...
  private static determineEngagementTrend(
    posts: ProcessedPost[]
  ): 'up' | 'down' | 'stable' {
    ...
    const recentAvg =
      recent.reduce((sum, p) => sum + p.engagement.engagementRate, 0) /
      recent.length
    const olderAvg =
      older.reduce((sum, p) => sum + p.engagement.engagementRate, 0) /
      older.length
  ...
  private static analyzeEngagementPatterns(
    ...
    return {
      morning_engagement: posts.filter(
        p => p.metadata.publishedAt.getHours() < 12
      ).length,
      afternoon_engagement: posts.filter(p => {
        const hour = p.metadata.publishedAt.getHours()
        return hour >= 12 && hour < 18
      }).length,
      evening_engagement: posts.filter(
        p => p.metadata.publishedAt.getHours() >= 18
      ).length,
    }
  }
  ...
  private static calculateSearchScore(
    ...
    // Tag match bonus
    if (post.metadata.tags.some(tag => tag.name.toLowerCase().includes(query)))
      score += 20
  ...
  private static calculateUserSearchScore(
    user: ProcessedUser,
    query: string
  ): number {
    let score = 0
