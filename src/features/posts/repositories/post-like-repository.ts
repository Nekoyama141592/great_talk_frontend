...
export class PostLikeRepository {
  ...
  async likePost(
    ...
    try {
      ...
      const postLikeRef = doc(
        collection(
          db,
          `public/v1/users/${targetUserId}/posts/${postId}/postLikes`
        ),
        currentUserId
      )
      const postLikeData: Omit<PostLike, 'createdAt'> & {
      ...
      const tokenData: Omit<PostLikeToken, 'createdAt'> & {
        createdAt: ReturnType<typeof serverTimestamp>
      } = {
        activeUid: currentUserId,
        createdAt: serverTimestamp(),
  ...
  async unlikePost(
    ...
    try {
      ...
      const tokenRef = doc(
        db,
        `private/v1/privateUsers/${currentUserId}/tokens/${tokenDoc.id}`
      )
      ...
      const postLikeRef = doc(
        db,
        `public/v1/users/${targetUserId}/posts/${postId}/postLikes/${currentUserId}`
      )
