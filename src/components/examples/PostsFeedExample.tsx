...
import { useDIKW, useContentRecommendations } from '../../layers/integration/hooks/use-dikw'
import { ProcessedPost, ProcessedUser, ProcessedAIInteraction } from '../../layers/information/models/processed-data'
...
export function PostsFeedExample({ className }: PostsFeedExampleProps) {
      ...
      const mockPosts: ProcessedPost[] = [
        {
          ...
          content: {
            ...
            description: 'Learn how to craft better prompts for more effective AI interactions.',
          ...
          ai: {
            systemPrompt: 'You are an expert AI assistant helping users learn prompt engineering...',
            responseCount: 34,
            averageResponseTime: 1200,
            conversationStarters: ['How do I improve my prompts?', 'What makes a good AI prompt?']
          }
        }
      ]
      ...
      const mockInteractions: ProcessedAIInteraction[] = [
        {
          ...
          conversation: {
            ...
            aiResponse: 'Here are some key strategies for better prompt engineering...',
            context: {}
          },
          technical: {
            responseTime: 1100,
  ...
  if (!uid) {
    return <div className="p-4 text-gray-600">Please log in to view personalized content.</div>
  }
  ...
  if (isProcessing) {
    return (
      <div className="p-4">
        ...
        <p className="text-sm text-gray-600 mt-2">Processing your personalized feed...</p>
      </div>
    )
  }
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      {lastUserExperience && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <h3 className="text-blue-800 font-medium mb-2">🧠 AI-Powered Insights</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            <div>
              <span className="font-medium text-blue-700">User Profile:</span>
              <p className="text-blue-600">
                Activity Score: {lastUserExperience.processedUser.social.activityScore}/100
              </p>
              <p className="text-blue-600">
                Influence: {lastUserExperience.processedUser.social.influenceLevel}
              </p>
            </div>
            <div>
              <span className="font-medium text-blue-700">Recommendations:</span>
              <p className="text-blue-600">
                {lastUserExperience.recommendations?.posts?.length || 0} personalized posts
              </p>
            </div>
            <div>
              <span className="font-medium text-blue-700">AI Strategy:</span>
              <p className="text-blue-600">
                {lastUserExperience.strategicDecisions?.aiStrategy?.model || 'standard'} mode
              </p>
            </div>
          </div>
        </div>
      ...
      {recommendations && (
        <div>
          ...
          <div className="space-y-4">
            {recommendations.posts?.slice(0, 5).map((scoredPost: any, index: number) => (
              <div key={scoredPost.post.postId} className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center space-x-2 mb-2">
                      <span className="font-medium text-gray-900">{scoredPost.post.author.displayName}</span>
                    ...
                    <div className="flex items-center space-x-4 text-sm text-gray-500 mb-3">
                      ...
                      <span>🎯 {scoredPost.post.engagement.engagementRate}% engagement</span>
                      <span>⭐ {scoredPost.post.quality.contentScore.toFixed(2)} quality</span>
                    </div>

                    {/* Recommendation Reasons */}
                    <div className="flex flex-wrap gap-2 mb-3">
                      {scoredPost.reasons.map((reason: string, reasonIndex: number) => (
                        <span
                          key={reasonIndex}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {scoredPost.post.metadata.tags.map((tag: any, tagIndex: number) => (
                        <span
                          key={tagIndex}
      ...
      {mockData?.allPosts && (
        <div>
          ...
          <div className="space-y-4">
            {mockData.allPosts.map((post, index) => (
              <div key={post.postId} className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                <div className="flex items-center space-x-2 mb-2">
                  <span className="font-medium text-gray-900">{post.author.displayName}</span>
                  {post.author.isVerified && <span className="text-blue-500">✓</span>}
                  {post.metadata.isTrending && <span className="text-orange-500">🔥</span>}
                </div>
                
                <h3 className="font-semibold text-lg text-gray-900 mb-2">{post.content.title}</h3>
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 text-sm">
        <h4 className="font-medium text-gray-800 mb-2">🏗️ DIKW Architecture Status</h4>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
          ...
          <div>
            <span className="block font-medium text-gray-700">Information Layer</span>
            <span className="text-green-600">✓ Processing</span>
          </div>
          <div>
            <span className="block font-medium text-gray-700">Knowledge Layer</span>
            <span className="text-green-600">✓ Recommendations</span>
          </div>
          <div>
            <span className="block font-medium text-gray-700">Wisdom Layer</span>
